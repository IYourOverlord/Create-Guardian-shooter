...
public class MachineSoulActionScreen extends BaseMachineSoulScreen<MachineSoulActionMenu> {
    ...
    @Override protected void renderContent(GuiGraphics g,int lx,int ty,int mx,int my){
        ...
        if(h)g.renderTooltip(font,Component.literal("Assign Redstone Link frequencies to the FIRE command"),mx,my); renderGhostItems(g,mx,my);
    }
    private void renderGhostItems(GuiGraphics g,int mx,int my){for(int i=0;i<2;i++){Slot s=menu.slots.get(i);ItemStack st=s.getItem();if(st.isEmpty())continue;int x=leftPos+(i==0?MachineSoulActionMenu.FREQ_X0:MachineSoulActionMenu.FREQ_X1),y=topPos+MachineSoulActionMenu.FIRE_SLOT_Y;RenderSystem.enableBlend();RenderSystem.setShaderColor(1,1,1,.5f);g.renderItem(st,x,y);RenderSystem.setShaderColor(1,1,1,1);RenderSystem.disableBlend();g.fill(x,y,x+16,y+16,COL_GHOST_OVERLAY);}}
    @Override public boolean mouseClicked(double mx,double my,int b){for(int i=0;i<2;i++){int x=leftPos+(i==0?MachineSoulActionMenu.FREQ_X0:MachineSoulActionMenu.FREQ_X1),y=topPos+MachineSoulActionMenu.FIRE_SLOT_Y;if(mx>=x&&mx<x+16&&my>=y&&my<y+16){ItemStack c=menu.getCarried();menu.setFreqItem(i,(b==1||c.isEmpty())?ItemStack.EMPTY:c);return true;}}return super.mouseClicked(mx,my,b);}
