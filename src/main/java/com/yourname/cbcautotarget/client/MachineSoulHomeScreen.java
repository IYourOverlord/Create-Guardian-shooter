...
public class MachineSoulHomeScreen extends AbstractContainerScreen<MachineSoulHomeMenu> {
    private static final int W=300, H=220, CX=120, CY=112, BUTTON_W=64, BUTTON_H=18;
    ...
    @Override protected void renderBg(GuiGraphics g,float pt,int mx,int my){
        ...
        float target=Mth.clamp(((mx-(lx+CX))/48f)*45f,-45f,45f);
        ...
        MachineSoulGuiSupport.renderGuardian(g,MachineSoulGuiSupport.guardian(),lx+CX,ty+CY,24,guardianYaw);
        ...
        int[][] pos={{18,62},{194,62},{194,142},{18,142}};
    ...
    @Override public boolean mouseClicked(double mx,double my,int b){if(b!=0)return super.mouseClicked(mx,my,b);int x=(int)mx,y=(int)my;
        ...
        int[][] pos={{18,62},{194,62},{194,142},{18,142}};for(int i=0;i<CARDS.length;i++)if(inside(x,y,leftPos+pos[i][0],topPos+pos[i][1],BUTTON_W,BUTTON_H)){PacketDistributor.sendToServer(new SwitchMachineSoulTabPacket(blockPos,CARDS[i].tab()));return true;}return super.mouseClicked(mx,my,b);
    }
